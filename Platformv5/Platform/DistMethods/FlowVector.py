...
def getRedistDict_Flow(VCont, n_keys, n_benches, bench_tot, n_queuelens, queuelen_tot):
...
def getRedistDict_Flow_3D(VCont, n_keys, n_benches, bench_tot, n_queuelens, queuelen_tot, n_latencies, latency_tot):
    ...
    sv = VCont.getStasisVector()  ## these values need to be 3 dimentional
    fv = VCont.getFlowVector()    ##as if [queuelength, bench, latency]
                                        # sv = [0.0, 0.0,0.5]                  #*
                                        # fv = [-1.0, 0.0, 0.5]                #*
    expprint("flow flow="+str(fv))                          
    expprint("flow stasis="+str(sv))                        
    ...
    for val in ds:
        ...
        dict_to_return[val[0]] = dict_to_return[val[0]] + 1
        extra = extra - 1
    dbgprint("flow redist dict="+str(dict_to_return))                    #expprint
